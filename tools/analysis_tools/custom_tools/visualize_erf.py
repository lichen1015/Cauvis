...
from mmdet.models import RemNet, YOLOv8CSPDarknet, ResNet
from timm.data.constants import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from mmdet.apis import DetInferencer
plt.rcParams["font.family"] = "Times New Roman"
import seaborn as sns

# config = "/mnt/workspace/code_remnet/RemNet-mmdet/self_config/yolov8/yolov8_n-300e_coco.py"
# config = "/mnt/workspace/code_remnet/RemNet-mmdet/self_config/remdet/remdet_n-300e_mscoco.py"
config = "/mnt/workspace/code_remnet/RemNet-mmdet/self_config/remdet/remdet_l-300e_coco.py"
save_path = "/output_dir/receptive_field"
# init_cfg = "/mnt/workspace/code_remnet/RemNet-mmdet/output_dir/yolo_series/yolov8n_0619_0.08bs32/best_coco_bbox_mAP_epoch_270.pth"
# init_cfg = "/mnt/workspace/code_remnet/RemNet-mmdet/output_dir/remdet_series/remdet_tiny/best_coco_bbox_mAP_epoch_280.pth"
init_cfg = "/mnt/workspace/code_remnet/RemNet-mmdet/output_dir/remdet_series/remdet_l/best_coco_bbox_mAP_epoch_290.pth"
save_name = "remdet_l"
...
class yolov8_erf:
    ...
    def __init__(self, weight, device, layer, dataset, num_images, save_path) -> None:
        ...
        self.model = DetInferencer(model=config, weights=init_cfg).model
        self.model.cuda()
        for p in self.model.parameters():
            p.requires_grad_(True)
        self.model.eval()
        optimizer = torch.optim.SGD(self.model.parameters(), lr=0, weight_decay=0)
        meter = AverageMeter()
        optimizer.zero_grad()

        if '-' in layer:
            layer_first, layer_second = layer.split('-')
            self.hooks.append(self.model.backbone[int(layer_first)].register_forward_hook(
                get_activation(self.feature, backbone_idx=int(layer_second))))
        else:
            self.hooks.append(self.model.backbone.stage4[1].register_forward_hook(get_activation(self.feature)))
...
def get_params():
    params = {
        ...
        'device': 'cuda:0',
        'layer': '8',  # string
        # 'dataset': '/mnt/datasets/VisDrone2019/VisDrone2019-DET-val/images',
        'dataset': '/mnt/datasets/MSCOCO2017/images/val2017',
        'num_images': 50,
        'save_path': save_path + f'/{save_name}_result.png'
    }
    return params
